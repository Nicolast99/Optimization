...
num_maquinas_A = int(input("Wie viele Maschinen von Typ A gibt es? "))
num_maquinas_B = int(input("Wie viele Maschinen von Typ B gibt es? "))
num_maquinas_C = int(input("Wie viele Maschinen von Typ C gibt es? "))
...
num_piezas = int(input("Wie viele Werkstücke gibt es? "))
...
for i in range(1, num_piezas + 1):
    print(f"\n Eingabe von Informationen für das Teil {i}")
    
    # Solicitar el orden de máquinas
    while True:
        orden_input = input("Geben Sie die Maschinenreihenfolge durch Bindestriche getrennt ein (z. B. A-B-C): ")
        orden_maquinas = validar_orden(orden_input, ['A', 'B', 'C'])
        if orden_maquinas:
            break
        else:
            print("Ungültige Bestellung. Stellen Sie sicher, dass Sie alle Maschinen nur einmal einbinden.")
    
    # Solicitar los tiempos de procesamiento según el orden de máquinas
    tiempos = {}
    for maquina in orden_maquinas:
        tiempo = int(input(f"Zeit an der Maschine {maquina} (Minuten, 0, wenn nicht auf diesem CMaschine verarbeitet): "))
...
def optimizar_con_ACO_tiempos(num_piezas, num_maquinas_A, num_maquinas_B, num_maquinas_C):
    ...
    for it in range(kmax):
        ...
        f_hist[it] = best_fit
        if it % 100 == 0:
            print(f"Iteration {it}: Totalzeit (fitness) = {best_fit}")
...
print(f"\n ACO : Die beste Reihenfolge ist {mejor_orden_aco} bei einer Gesamtzeit {mejor_tiempo_aco} Minuten")
...
print(f"\nIG: Die beste Reihenfolge ist {mejor_orden_ig} bei einer Gesamtzeit {mejor_tiempo_ig} Minuten")
print("Aufgaben IG:")
